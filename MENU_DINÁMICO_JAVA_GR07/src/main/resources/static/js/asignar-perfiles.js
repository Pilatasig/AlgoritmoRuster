...
async function cargarTablaAsignados(codigoPerfil) {
    try {
        ...
        const tieneX24 = typeof PERMISOS !== 'undefined' && PERMISOS.includes('X24');
        ...
        data.forEach(item => {
            ...
            if (tieneX24) acciones += `<button class="btn btn-sm btn-outline-danger" onclick="removerUsuario('${codigoPerfil}', '${u.id}')" title="Remover perfil"><i class="bi bi-x-circle"></i></button>`;
            acciones += '</div>';
...
async function cargarTablaDisponibles(codigoPerfil) {
    try {
        ...
        const tieneX21 = typeof PERMISOS !== 'undefined' && PERMISOS.includes('X21');

        data.forEach(u => {
            const emp = u.empleado || {};
            let acciones = '<div class="d-flex justify-content-center gap-1">';
            if (tieneX21) acciones += `<button class="btn btn-sm btn-outline-success" onclick="asignarUsuario('${codigoPerfil}', '${u.id}')" title="Asignar perfil"><i class="bi bi-plus-circle"></i></button>`;
            acciones += '</div>';

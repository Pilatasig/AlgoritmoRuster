...
async function cargarArbol() {
    ...
    try {
        ...
        const puedeX31 = typeof PERMISOS !== 'undefined' && PERMISOS.includes('X31');

        container.innerHTML = '';
        panel.style.display = 'block';
        btnGuardar.style.display = puedeX31 ? 'block' : 'none';
        ...
        arbol.forEach(sis => {
            ...
            if (sis.modulos) {
                ...
                sis.modulos.forEach(mod => {
                    const modDiv = crearNodoModulo(mod, codigoPerfil, puedeX31);
...
function crearNodoModulo(mod, codigoPerfil, editable) {
    const div = document.createElement('div');
    ...
    if (mod.hijas) {
        mod.hijas.forEach(hija => {
            ...
            hijaDiv.innerHTML = `
                <input type="checkbox" class="hija-chk me-2" data-perfil="${codigoPerfil}" data-opcion="${hija.codigo}" ${hija.asignada ? 'checked' : ''} ${disabledAttr}>
